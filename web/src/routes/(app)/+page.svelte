...
<div class="w-full min-h-screen flex flex-col bg-primary-50 text-slate-900 overflow-x-hidden">

	<!-- Header -->
	<header class="shrink-0 pt-14 px-4 relative z-10">
		<div class="flex items-center justify-between">
			<div class="flex items-center gap-3">
				<div class="size-11 rounded-2xl bg-primary-500 shadow-md flex items-center justify-center">
				...
				<div class="leading-tight">
					<p class="text-sm font-semibold text-slate-900 text-balance">Split Bill</p>
					<p class="text-xs text-slate-600 text-pretty">Make it fair in seconds</p>
				</div>
			</div>
			<div class="flex items-center gap-2">
				<button type="button" class="size-11 rounded-2xl bg-white shadow-md border border-slate-200 flex items-center justify-center active:scale-[0.99]">
	...
	<main class="flex-1 overflow-y-auto px-4 pt-5 pb-[120px] relative z-0 no-scrollbar">
		...
		<section class="rounded-3xl bg-white shadow-md border border-slate-200 p-5 relative overflow-hidden" style="max-width: calc(100vw - 2rem);">
			...
			<div class="flex items-start justify-between gap-4 mb-5">
				<div class="flex-1 min-w-0">
					...
					<button class="mb-3 inline-flex items-center gap-1.5 rounded-full bg-slate-100 pl-3 pr-2 py-1 text-xs font-bold text-slate-700 hover:bg-slate-200 active:scale-95 transition-transform">
						<span class="size-4 rounded-full bg-white flex items-center justify-center shadow-sm text-xs">Rp</span>
						IDR
					...
					<h1 class="text-2xl font-extrabold tracking-tight text-slate-900 leading-tight text-balance">Split bills easily<br>with friends</h1>
				</div>
				<div class="shrink-0">
					<div class="size-14 rounded-3xl bg-primary-50 border border-primary-500/20 flex items-center justify-center">
				...
				<div class="flex gap-3 overflow-x-auto pb-2 no-scrollbar">
					{#each templates as template}
						<button class="shrink-0 flex flex-col items-center gap-2 w-[72px] group">
							<div class="size-[52px] rounded-2xl bg-{template.color}-50 border border-{template.color}-100 text-{template.color}-500 flex items-center justify-center group-active:scale-95 transition-transform">
					...
					{/each}
					<button class="shrink-0 flex flex-col items-center gap-2 w-[72px] group">
						<div class="size-[52px] rounded-2xl bg-slate-50 border border-slate-100 text-slate-400 flex items-center justify-center group-active:scale-95 transition-transform">
			...
			<div class="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
				<div class="shrink-0 rounded-2xl bg-primary-50 border border-primary-500/15 px-3 py-2 min-w-[120px]">
					<p class="text-xs text-slate-600 text-pretty">This month</p>
					<p class="text-sm font-bold text-slate-900 tabular-nums">Rp 3.850k</p>
				</div>
				<div class="shrink-0 rounded-2xl bg-emerald-50 border border-emerald-500/15 px-3 py-2 min-w-[100px]">
					<p class="text-xs text-slate-600 text-pretty">Settled</p>
					<p class="text-sm font-bold text-slate-900 tabular-nums">{settledCount} of {totalCount}</p>
				</div>
				<div class="shrink-0 rounded-2xl bg-amber-50 border border-amber-500/15 px-3 py-2 min-w-[100px]">
					<p class="text-xs text-slate-600 text-pretty">Pending</p>
					<p class="text-sm font-bold text-slate-900 tabular-nums">{pendingCount} splits</p>
				</div>
			</div>
		</section>
		...
		<section class="mt-6">
			<div class="flex items-center justify-between mb-3">
				<h3 class="text-lg font-bold text-slate-900 text-balance">Your Groups</h3>
				...
				{#each groups as group}
					<a href="#group-{group.id}" class="shrink-0 w-40 p-4 rounded-3xl bg-white shadow-sm border border-slate-200 active:scale-[0.98] transition-transform block">
						<div class="flex items-center gap-3 mb-3">
							<div class="size-10 rounded-xl bg-{group.color}-100 text-{group.color}-600 flex items-center justify-center">
								{@html getIcon(group.icon)}
							</div>
							<div>
								<p class="text-sm font-bold text-slate-900 text-balance">{group.name}</p>
								<p class="text-xs text-slate-500 text-pretty">{group.members} members</p>
							</div>
						</div>
						<p class="text-xs text-slate-400 font-medium text-pretty">Last split {group.lastActivity}</p>
					</a>
				{/each}
			</div>
		</section>
		...
		<section class="mt-2 mb-6">
			<h3 class="mb-3 text-sm font-bold text-slate-900 text-balance">Activity</h3>
			<div class="space-y-2">
				{#each activities as activity}
					<div class="flex items-center gap-3 p-3 rounded-2xl bg-white/60 border border-white/50 backdrop-blur-sm">
						<div class="size-8 rounded-full bg-{activity.personColor}-200 border border-white flex items-center justify-center shrink-0">
							<span class="text-xs font-bold text-{activity.personColor}-800">{activity.person[0]}</span>
						</div>
						<p class="text-xs text-slate-700 leading-snug flex-1 text-pretty"><span class="font-bold text-slate-900">{activity.person}</span> {activity.action} <span class="font-semibold">{activity.target}</span>.</p>
						<span class="text-xs text-slate-400 tabular-nums">{activity.time}</span>
					</div>
				{/each}
			</div>
		</section>
		...
		<section class="mt-4 mb-8">
			<div class="rounded-3xl bg-slate-900 text-white p-5 shadow-lg">
				<div class="flex items-start justify-between">
					<div>
						<p class="text-xs font-medium text-slate-400 mb-1 text-pretty">Total Spent (Feb)</p>
						<h2 class="text-2xl font-bold text-balance tabular-nums">Rp 3.850k</h2>
					</div>
				...
				<div class="mt-6 flex items-end justify-between gap-2 h-24 px-1">
					<div class="w-full flex flex-col justify-end gap-1 group cursor-pointer">
						...
						<span class="text-xs text-slate-500 text-center tabular-nums">Dec</span>
					</div>
					<div class="w-full flex flex-col justify-end gap-1 group cursor-pointer">
						<div class="w-full bg-slate-800 rounded-t-lg h-[65%] group-hover:bg-primary-500 transition-colors"></div>
						<span class="text-xs text-slate-500 text-center tabular-nums">Jan</span>
					</div>
					<div class="w-full flex flex-col justify-end gap-1 group cursor-pointer">
						<div class="w-full bg-primary-500 rounded-t-lg h-[85%]"></div>
						<span class="text-xs text-white font-bold text-center tabular-nums">Feb</span>
					</div>
					<div class="w-full flex flex-col justify-end gap-1 group cursor-pointer">
						...
						<span class="text-xs text-slate-500 text-center tabular-nums">Mar</span>
					</div>
				</div>
			</div>
		</section>
		...
		<section class="space-y-3">
			<div class="flex items-center justify-between">
				<p class="text-lg font-bold text-slate-900 text-balance">Recent Splits</p>
			...
			{#each recentSplits as split}
				<article class="rounded-3xl bg-white shadow-md border border-slate-200 p-4">
					<div class="flex items-start gap-3">
						<div class="size-12 rounded-2xl bg-{split.color}-50 border border-{split.color}-500/15 flex items-center justify-center shrink-0">
							{@html getIcon(split.icon)}
						</div>
						<div class="min-w-0 flex-1">
							<div class="flex items-start justify-between gap-3">
								<div class="min-w-0 flex-1">
									<p class="text-sm font-semibold text-slate-900 truncate text-balance">{split.title}</p>
									<p class="mt-0.5 text-xs text-slate-600 text-pretty tabular-nums">{split.date} • {split.people} people</p>
								</div>
								<div class="text-right shrink-0">
									<p class="text-sm font-extrabold text-slate-900 tabular-nums">{split.amount}</p>
					...
					<div class="mt-3 flex items-center justify-between gap-3 pl-[60px]">
						<div class="flex -space-x-2">
							<div class="size-7 rounded-full bg-slate-200 border-2 border-white flex items-center justify-center text-xs font-bold text-slate-600">A</div>
							<div class="size-7 rounded-full bg-sky-200 border-2 border-white flex items-center justify-center text-xs font-bold text-slate-600">M</div>
							<div class="size-7 rounded-full bg-emerald-200 border-2 border-white flex items-center justify-center text-xs font-bold text-slate-600">J</div>
						</div>
		...
		<section class="mt-6 rounded-3xl bg-indigo-600 shadow-md p-4 text-white">
			<div class="flex items-start gap-3">
				<div class="size-10 rounded-xl bg-white/20 flex items-center justify-center shadow-sm shrink-0">
				...
				<div class="min-w-0 flex-1">
					<p class="text-sm font-bold text-balance">Scanning Tips</p>
					<p class="mt-1 text-xs text-indigo-100 text-pretty">Ensure good lighting for best results. We'll handle the math!</p>
				</div>
			</div>
		</section>
	</main>
