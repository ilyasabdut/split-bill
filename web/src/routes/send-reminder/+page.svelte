...
<div class="w-full min-h-dvh flex flex-col bg-slate-50 text-slate-900 font-sans">
	...
	<header class="shrink-0 px-4" style="padding-top: max(env(safe-area-inset-top), 3rem);">
		<div class="flex items-center justify-between max-w-lg mx-auto">
			...
			<div class="text-center">
				<div class="text-xs font-bold text-slate-500 uppercase tracking-wider">Payment</div>
				<h1 class="text-lg font-black tracking-tight text-slate-900">Send Reminder</h1>
			</div>
	...
	<main class="flex-1 overflow-y-auto px-4 py-4 space-y-5 max-w-lg mx-auto w-full" style="padding-bottom: calc(env(safe-area-inset-bottom, 0px) + 2rem);">
		...
		{:else}
			...
			<!-- Select Recipients -->
			<section>
				<h2 class="text-sm font-bold text-slate-900 mb-3">Select Recipients</h2>
				<div class="rounded-3xl bg-white shadow-md border border-slate-200 overflow-hidden">
					{#each members as member, index}
						<button
							type="button"
							onclick={() => toggleMember(member.id)}
							class="w-full p-4 flex items-center gap-3 text-left {index < members.length - 1 ? 'border-b border-slate-100' : ''} {member.balance === 0 ? 'opacity-50' : ''}"
						...
						>
							...
							{#if member.balance < 0}
								<div class="text-right">
									...
									<p class="text-[10px] text-slate-400">owes you</p>
								</div>
								<div class="h-6 w-6 rounded-full {member.selected ? 'bg-amber-500' : 'bg-slate-200'} flex items-center justify-center transition-colors">
									{#if member.selected}
			...
			<!-- Custom Message -->
			<section>
				<h2 class="text-sm font-bold text-slate-900 mb-3">Custom Message <span class="text-slate-400 font-normal">(optional)</span></h2>
				<div class="rounded-3xl bg-white shadow-md border border-slate-200 p-4">
					<textarea
						bind:value={customMessage}
						placeholder="Add a personal note to the reminder..."
						rows="3"
						class="w-full bg-transparent border-none p-0 text-sm text-slate-900 focus:ring-0 placeholder:text-slate-300 resize-none"
					></textarea>
				</div>
			</section>

			<!-- Preview Email -->
			<section class="rounded-3xl bg-gradient-to-br from-slate-800 to-slate-900 p-4 text-white">
				<div class="flex items-center gap-2 mb-3">
					...
					<span class="text-xs font-bold text-slate-400 uppercase tracking-wider">Email Preview</span>
				</div>
				<div class="bg-white/10 backdrop-blur-sm rounded-2xl p-4 text-sm">
					<p class="font-bold text-white mb-2">Hi {"{name}"}!</p>
					...
					<p class="text-slate-300 mt-3">Please settle up when you get a chance!</p>
				</div>
			</section>
