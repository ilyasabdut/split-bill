...
<div class="w-full min-h-dvh flex flex-col bg-slate-50 text-slate-900 font-sans">
	...
	<main class="flex-1 overflow-y-auto px-4 py-4 space-y-5 max-w-lg mx-auto w-full" style="padding-bottom: calc(env(safe-area-inset-bottom, 0px) + 2rem);">
		...
		<section class="rounded-3xl bg-gradient-to-br from-indigo-500 to-violet-600 shadow-xl shadow-indigo-500/25 p-5 text-white relative overflow-hidden">
			...
			<div class="relative z-10">
				<div class="flex items-start justify-between mb-4">
					...
					<div class="h-12 w-12 rounded-2xl bg-white/20 backdrop-blur-sm flex items-center justify-center">
		...
		<section class="grid grid-cols-2 gap-3">
			<a href="/split" class="h-20 rounded-2xl bg-white shadow-md border border-slate-200 p-4 flex flex-col items-center justify-center gap-2 active:scale-[0.98] transition-transform">
				...
				<span class="text-sm font-bold text-slate-900">New Split</span>
			</a>
			<a href="/receipt" class="h-20 rounded-2xl bg-white shadow-md border border-slate-200 p-4 flex flex-col items-center justify-center gap-2 active:scale-[0.98] transition-transform">
				...
				<span class="text-sm font-bold text-slate-900">Scan Receipt</span>
			</a>
		</section>
		...
		<section>
			<div class="flex items-center justify-between mb-3">
				...
				<span class="text-xs font-bold text-slate-500">{group.members.length} people</span>
			</div>

			<div class="rounded-3xl bg-white shadow-md border border-slate-200 overflow-hidden">
				{#each group.members as member, index}
					<div class="p-4 flex items-center gap-3 {index < group.members.length - 1 ? 'border-b border-slate-100' : ''}">
						...
						<div class="flex-1 min-w-0">
							<div class="flex items-center gap-2">
								...
								{#if member.role === 'admin'}
									<span class="text-[10px] font-bold bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded">Admin</span>
								{/if}
							</div>
						...
						<div class="text-right">
							{#if member.balance > 0}
								...
								<p class="text-[10px] text-slate-400">owed to you</p>
							{:else if member.balance < 0}
								<p class="text-sm font-bold text-rose-600">{formatCurrency(member.balance)}</p>
								<p class="text-[10px] text-slate-400">you owe</p>
							{:else}
								<p class="text-sm font-bold text-slate-400">Settled</p>
							{/if}
						</div>
					</div>
				{/each}
				...
				<div class="p-3 bg-slate-50 border-t border-slate-100">
					<button type="button" class="w-full h-10 rounded-xl border-2 border-dashed border-slate-300 text-slate-500 font-bold text-sm flex items-center justify-center gap-2 hover:border-indigo-300 hover:text-indigo-600 hover:bg-indigo-50 transition-all active:scale-[0.99]">
		...
		<section>
			<div class="flex items-center justify-between mb-3">
				<h2 class="text-sm font-bold text-slate-900">Recent Receipts</h2>
				<a href="#" class="text-xs font-bold text-indigo-600">View all</a>
			</div>

			<div class="space-y-3">
				{#each group.receipts as receipt}
					<a href="/split/{receipt.id}" class="block rounded-2xl bg-white shadow-md border border-slate-200 p-4 active:scale-[0.99] transition-transform">
						<div class="flex items-center gap-3">
							...
							<div class="flex-1 min-w-0">
								<p class="text-sm font-bold text-slate-900 truncate">{receipt.title}</p>
								<p class="text-xs text-slate-500 mt-0.5">{receipt.date} • Paid by {receipt.paidBy}</p>
							</div>
							<div class="text-right">
								<p class="text-sm font-bold text-slate-900">{formatCurrency(receipt.amount)}</p>
		...
		<!-- Send Reminder -->
		<section class="rounded-3xl bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 p-4">
			<div class="flex items-start gap-3">
				<div class="h-10 w-10 rounded-2xl bg-white/80 border border-amber-200 flex items-center justify-center shadow-sm">
				...
				<div class="min-w-0 flex-1">
					<p class="text-sm font-bold text-slate-900">Payment Reminder</p>
					<p class="text-xs text-slate-600 mt-0.5">Send reminders to members with pending balances.</p>
		...
		<section class="rounded-3xl bg-white shadow-md border border-slate-200 overflow-hidden">
			<div class="px-5 py-3 border-b border-slate-100 bg-slate-50/50">
				<h2 class="text-sm font-bold text-slate-900">Settlement Summary</h2>
			</div>
					...
					{:else if member.balance > 0 && member.id !== '1'}
						<div class="flex items-center justify-between p-3 bg-emerald-50 rounded-xl border border-emerald-100">
							<div class="flex items-center gap-2">
								...
								<span class="text-sm font-medium text-slate-900">{member.name} owes you</span>
							</div>
