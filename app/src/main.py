...
from constants import BANK_NAMES, E_WALLET_PROVIDERS, MAX_IMAGE_SIZE_BYTES
...
if "processed_image_bytes_for_minio_base64" not in st.session_state:
    st.session_state.processed_image_bytes_for_minio_base64 = None  # Now base64 string
...
def main_app_flow():
    ...
    if st.session_state.current_step == 0:
        ...
        if uploaded_file is not None:
            ...
            if (
                ...
                with st.spinner("⚙️ Processing receipt..."):
                    try:
                        files = {
                            "file": (
                                uploaded_file.name,
                        ...
                        if error_detail:
                            ...
                            if status_code == 400 and "receipt" in normalized_detail:
                                ...
                                st.warning(
                                    "The AI service had trouble classifying the receipt. Please try again with a clearer photo or retry later."
                                )
                ...
                and "Error" not in st.session_state.parsed_data
            ):
                if st.button("Proceed with current receipt", type="primary"):
                    st.session_state.current_step = 1
                    st.rerun()
    ...
    if data_source and "Error" not in data_source:
        ...
        if isinstance(data_source.get("tax_details"), list):
            for tax_item in data_source.get("tax_details", []):
                if isinstance(tax_item, dict) and isinstance(
                    tax_item.get("tax_amount"), (int, float)
                ):
                    tax_sum += tax_item.get("tax_amount", 0.0)

    should_show_receipt_expander = bool(
        data_source
    ...
    if should_show_receipt_expander:
        ...
        with st.expander("Receipt Details", expanded=expanded_state):
            ...
            if receipt_date or receipt_time_val:
                st.write(
                    f"🗓️ **Date:** {receipt_date or 'N/A'} | 🕒 **Time:** {receipt_time_val or 'N/A'}"
                )
    ...
    if st.session_state.current_step == 2 and not is_view_mode:
        ...
        with col_next2:
            ...
            if (
                ...
                st.caption("⚠️ Please assign all items if not splitting evenly.")
    ...
    if st.session_state.current_step == 3 and not is_view_mode:
        ...
        initial_tax = (
            float(
                st.session_state.parsed_data.get("tax_details", [{}])[0].get(
                    "tax_amount"
                )
                or 0.0
            )
        ...
        if payment_method == "Bank":
            st.session_state.payment_details["bank_name"] = st.selectbox(
                ...
                index=(
                    BANK_NAMES.index(
                        st.session_state.payment_details.get("bank_name")
                        or BANK_NAMES[0]
                    )
        ...
        with col_back3:
            if st.button("⬅️ Back to Assign Items", use_container_width=True):
                reset_to_step(2)
                st.rerun()
        with col_calc:
            if st.button(
                ...
                if (
                    ...
                    with st.spinner("Calculating split and generating link..."):
                        try:
                            ...
                            st.session_state.split_results = api_response[
                                "split_results"
                            ]
                            ...
                            if (
                                response is not None and response.status_code
    ...
    if st.session_state.current_step == 4:
        ...
        if not is_view_mode:
            if st.button("⬅️ Adjust Split Details", use_container_width=True):
                ...
                if (
                    st.session_state.item_assignments
                    or not st.session_state.split_evenly
                ):
                    reset_to_step(2)
                else:
                    reset_to_step(3)
...
if __name__ == "__main__":
    ...
    if shared_split_id_from_url and (
        ...
        print(
            f"URL has split_id: {shared_split_id_from_url}. Current view_split_id: {st.session_state.view_split_id}"
        )
        reset_app_state_full()  # Reset state before loading a potentially new shared link
        st.session_state.view_split_id = (
            shared_split_id_from_url  # Mark that we are attempting to view this
        )
        ...
        loaded_data_dict = load_shared_split_data_from_api(
            shared_split_id_from_url
        )  # Use the new API function
        if loaded_data_dict:
            print(
                f"Successfully loaded data for shared split_id: {shared_split_id_from_url}"
            )
            ...
            st.session_state.share_link = loaded_data_dict.get(
                "share_link"
            )  # Also load the share link itself
            ...
            print(
                f"Failed to load data for shared split_id: {shared_split_id_from_url}. Resetting."
            )
