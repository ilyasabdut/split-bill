...
def get_openrouter_config() -> OpenRouterConfig:
    ...
    return OpenRouterConfig(api_key=api_key, model_name=model_name, referer=referer, title=title)
...
def _detect_mime_type(image_bytes: bytes) -> str:
    ...
    try:
        ...
        logger.debug(f"Could not detect image format for payload: {exc}. Defaulting to JPEG MIME type.")
        img_format = "JPEG"
    ...
    *,
    ...
    if response.status_code >= 400:
        ...
        try:
            ...
            raise RuntimeError(f"OpenRouter API error {response.status_code}: {response.text}") from exc

    try:
        return response.json()
    except ValueError as exc:  # JSON decoding error
        raise RuntimeError(f"Failed to decode OpenRouter response as JSON: {exc}") from exc
...
def _parse_json_response(raw_text: str) -> Dict[str, Any]:
    ...
    if cleaned_text.startswith("```"):
        lines = [line for line in cleaned_text.splitlines() if not line.strip().startswith("```")]
        cleaned_text = "\n".join(lines).strip()
    ...
    if start != -1 and end != -1 and start < end:
        ...
        try:
            ...
            raise ValueError(f"Model response was not valid JSON: {exc}\nResponse text: {cleaned_text}") from exc

    raise ValueError(f"Model response was not valid JSON. Response text: {cleaned_text}")
...
def extract_receipt_data(image_bytes: bytes) -> Dict[str, Any]:
    start_time = time.time()
    logger.info(f"Starting receipt data extraction via OpenRouter at {time.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    try:
        ...
        except Exception as validation_error:  # noqa: BLE001 - log validation issues and return raw data
