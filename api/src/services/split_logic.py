...
import logging
import re
from typing import Any, Dict, List  # Python 3.9+
...
def parse_quantity(num_str: str) -> float | None:  # Python 3.10+ type hint
    ...
    if not isinstance(num_str, str):
        return None
    num_str_cleaned = (
        num_str.replace("x", "", 1).replace("X", "", 1).strip().lower()
    )  # Replace only first 'x'
    match = re.match(
        r"^(\d+(?:[.,]\d*)?)$", num_str_cleaned
    )  # Allows 1 or 1.0 or 1,0 or 1.
    if match:
...
def clean_and_convert_number(
    num_str: str | int | float, is_quantity: bool = False
) -> float | None:  # Python 3.10+
    ...
    if is_quantity:
        return parse_quantity(num_str_stripped)
        ...
        if "," in num_str_stripped and num_str_stripped.count(".") <= 1:
            try:  # Attempt to interpret comma as decimal if it makes sense
                temp_str = num_str_stripped.replace(".", "").replace(
                    ",", "."
                )  # 1.234,56 -> 1234.56
...
def calculate_split(
    ...
    if split_evenly_flag:
        logger.info(
            f"Splitting bill evenly. Initial overall subtotal: {overall_subtotal_for_even_split}, Discount: {actual_total_discount}"
        )
        subtotal_after_discount = (
            overall_subtotal_for_even_split - actual_total_discount
        )
        ...
        cost_per_person_even = (
            subtotal_basis_for_tax_tip / len(person_names) if person_names else 0
        )
        ...
        for person in person_names:
            ...
            split_results[person]["items"].append(
                {
                    "item": "Even Share of Bill (after discount)",
                    "qty_share": 1.0,
                    "price_per_unit": cost_per_person_even,
                    "share_cost": cost_per_person_even,
                }
            )
        ...
        for assignment in assignments:
            ...
            line_item_total_price = (
                clean_and_convert_number(item_details.get("price", "0.0")) or 0.0
            )

            if (
                not assigned_to_list
            ):  # Unassigned items don't contribute to anyone's subtotal or the bill's subtotal
                logger.info(
                    f"Info: Item '{item_name}' not assigned, cost not included in split."
                )
                continue
            ...
            price_per_single_unit = (
                line_item_total_price / line_item_quantity
                if line_item_quantity > 0
                else line_item_total_price
            )
            cost_per_person_for_this_item = (
                line_item_total_price / len(assigned_to_list) if assigned_to_list else 0
            )
            quantity_share_per_person = (
                line_item_quantity / len(assigned_to_list) if assigned_to_list else 0
            )
            ...
            for person in assigned_to_list:
                if person in split_results:
                    split_results[person]["items"].append(
                        {
                            "item": item_name,
                            "qty_share": quantity_share_per_person,
                            "price_per_unit": price_per_single_unit,
                            "share_cost": cost_per_person_for_this_item,
                        }
                    )
        ...
        subtotal_basis_for_tax_tip = (
            current_calculated_subtotal_from_items  # Start with sum of assigned items
        )
        if actual_total_discount > 0 and current_calculated_subtotal_from_items > 0:
            logger.info(
                f"Applying total discount of {actual_total_discount} proportionally to item-based subtotals."
            )
            for person in person_names:
                person_subtotal_before_discount = split_results[person]["subtotal"]
                # Proportion based on their share of the item costs
                proportion_of_subtotal = (
                    person_subtotal_before_discount
                    / current_calculated_subtotal_from_items
                    if current_calculated_subtotal_from_items > 0
                    else 0
                )
                person_discount_share = actual_total_discount * proportion_of_subtotal
                split_results[person]["subtotal"] -= person_discount_share
                if split_results[person]["subtotal"] < 0:
                    split_results[person]["subtotal"] = 0.0
            subtotal_basis_for_tax_tip -= (
                actual_total_discount  # Reduce the basis for tax/tip
            )
            ...
            logger.info(
                f"Note: Discount of {actual_total_discount} exists but no item costs to apply it against proportionally."
            )
    ...
    if subtotal_basis_for_tax_tip == 0:
        if tax_amount > 0 or tip_amount > 0:
            logger.info(
                "Subtotal basis for tax/tip is zero, distributing tax/tip evenly."
            )
        ...
        for person_name in person_names:
            ...
            proportion = (
                person_subtotal_after_discount / subtotal_basis_for_tax_tip
                if subtotal_basis_for_tax_tip > 0
                else 0
            )
...
if __name__ == "__main__":
    ...
    sample_assignments = [
        {
            "item_details": {"item": "Burger A", "qty": "1", "price": "10.00"},
            "assigned_to": ["Alice"],
        },
        {
            "item_details": {"item": "Burger B", "qty": "1", "price": "12.00"},
            "assigned_to": ["Bob"],
        },
        {
            "item_details": {"item": "Fries", "qty": "1", "price": "5.00"},
    ...
    logger.info("--- Test: Individual Items, No Discount ---")
    results_no_disc = calculate_split(sample_assignments, tax_str, tip_str, people)
    logger.info(json.dumps(results_no_disc, indent=2))
    ...
    results_with_disc = calculate_split(
        sample_assignments, "2.00", tip_str, people, total_discount_amount=7.00
    )
    logger.info(json.dumps(results_with_disc, indent=2))
    ...
    results_even_no_disc = calculate_split(
        [],
        tax_str,
        tip_str,
        people,
    ...
    logger.info(json.dumps(results_even_no_disc, indent=2))
    ...
    # Tax = 10% of 20 = 2.00
    results_even_with_disc = calculate_split(
        [],
        "2.00",
        tip_str,
        people,
    ...
    logger.info(json.dumps(results_even_with_disc, indent=2))
