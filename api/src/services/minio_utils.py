...
MINIO_ENDPOINT = os.environ.get(
    "MINIO_ENDPOINT", "158.179.26.94:9000"
)  # Ensure API PORT
...
minio_client_instance = (
    None  # Renamed to avoid conflict if minio_client is used elsewhere
)
...
def get_metadata_from_minio(base_object_name: str) -> Optional[Dict[str, Any]]:
    ...
    if json_bytes:
        try:
            ...
            print(
                f"Unexpected error processing metadata from MinIO '{object_name_with_prefix}': {e}"
            )
            return None
    return None
