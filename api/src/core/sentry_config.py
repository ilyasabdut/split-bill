...
class SentryTracker:
    ...
    def initialize(self):
        ...
        if not SENTRY_AVAILABLE or not self.dsn:
            print("Sentry not initialized - DSN not available or sentry_sdk not installed")
            return False
    ...
    def _before_send(self, event, hint):
        ...
        if event.get('transaction') == '/health' or event.get('transaction') == '/health/':
            return None
    ...
    def track_performance(self, operation_name: str, start_time: float, success: bool = True, context: Dict[str, Any] = None):
        ...
        with sentry_sdk.configure_scope() as scope:
            ...
            if success:
                sentry_sdk.capture_message(f"Performance: {operation_name} completed in {duration:.2f}ms", level="info")
            else:
                sentry_sdk.capture_message(f"Performance: {operation_name} failed after {duration:.2f}ms", level="error")
...
def track_performance(operation_name: str, start_time: float, success: bool = True, context: Dict[str, Any] = None):
    """Track performance metrics."""
