...
# Import shared image processing service (with fallback)
try:
    from services.image_service import compress_image
except ImportError:
    # Fallback if imports don't work in all contexts
    def compress_image(
        image_bytes, target_size_bytes=MAX_IMAGE_SIZE_BYTES, quality=90, min_quality=70
    ):
...
def load_shared_split_data(split_id: str) -> dict[str, Any] | None:
    ...
    if metadata:
        ...
        metadata["image_bytes_for_display"] = (
            image_bytes_for_display if image_bytes_for_display else None
        )
        return metadata
    else:
        return None
...
class CalculateSplitRequest(BaseModel):
    ...
    extracted_total_discount: float
    processed_image_bytes_for_minio_base64: str | None = (
        None  # Base64 encoded image for MinIO upload
    )
...
class SharedSplitDataResponse(BaseModel):
    ...
    image_bytes_for_display_base64: str | None = (
        None  # Base64 encoded image for display
    )
    notes_text: str
...
async def upload_receipt(
    ...
    if file.size > MAX_IMAGE_SIZE_BYTES:
        raise HTTPException(
            status_code=400,
            detail=f"Image too large ({file.size / (1024*1024):.2f} MB). Max {MAX_IMAGE_SIZE_MB} MB.",
        )
    ...
    if len(raw_image_bytes) > MAX_IMAGE_SIZE_BYTES:
        raise HTTPException(
            status_code=400,
            detail=f"Image too large ({len(raw_image_bytes) / (1024*1024):.2f} MB). Max {MAX_IMAGE_SIZE_MB} MB.",
        )
    ...
    elif not parsed_data_dict.get("line_items") and not parsed_data_dict.get(
        "total_amount"
    ):
        raise HTTPException(
            status_code=400,
    ...
    extracted_subtotal_from_gemini = (
        split_logic.clean_and_convert_number(subtotal_from_gemini) or 0.0
    )
    ...
    if isinstance(parsed_data_dict.get("discounts"), list):
        for disc in parsed_data_dict.get("discounts", []):
            total_discount += (
                split_logic.clean_and_convert_number(disc.get("amount")) or 0.0
            )
...
async def calculate_split_endpoint(
    ...
    idempotency_key_material = {
        "image_bytes_hash": (
            hashlib.sha256(
                base64.b64decode(request.processed_image_bytes_for_minio_base64) or b""
            ).hexdigest()
    ...
    if existing_metadata and existing_metadata.get("share_link"):
        print(
            f"Using existing share link for split_id {split_id}: {existing_metadata['share_link']}"
        )
        ...
        if request.processed_image_bytes_for_minio_base64:
            ...
            full_image_obj_name = minio_utils.upload_image_to_minio(
            ...
            if full_image_obj_name:
                ...
                print(
                    "Failed to save receipt image to cloud."
        ...
        metadata_to_save = {
            ...
            "item_assignments": [
                a.model_dump() for a in final_assignments_for_calc
        ...
        meta_upload_obj_name = minio_utils.upload_metadata_to_minio(
            metadata_to_save, split_id
        )
        if not meta_upload_obj_name:
            print(
                f"Failed to save split metadata for {split_id}."
            )  # Log, but don't fail the whole request
...
async def view_split(
    ...
    if loaded_data_dict.get("image_bytes_for_display"):
        image_bytes_for_display_base64 = base64.b64encode(
            loaded_data_dict["image_bytes_for_display"]
        ).decode("utf-8")
